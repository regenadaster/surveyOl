(function(){
  ...
  sy.createOptDiv=function(_opt){
      ...
      bFatherId=sy.getFirstDivId(_opt.getId());
	  if(_opt.getType()==0){
	    childOpt.html('<input type="radio" '+'name="'+bFatherId+'_optname">'+'<span contentEditable="true">'+_opt.getDescript()+'</span>');
	  }
	  if(_opt.getType()==1){
		childOpt.html('<input type="checkbox" '+'name="'+bFatherId+'_optname">'+'<span contentEditable="true">'+_opt.getDescript()+'</span>');
	  }
	  if(_opt.getType()==2){
		childOpt.html('<input type="radio" '+'name="'+bFatherId+'_optname">'+'<span contentEditable="true">'+_opt.getDescript()+'</span>');
	  }
  ...
  sy.appendBlock=function(_block){
	...
	if(!_block.getDraw()){
	  ...
	  childDiv=$('<div class="col-md-7 block col-md-offset-1"></div>');
	  childDiv.html('<div contentEditable="true">'+num+_block.getTitle()+'</div>');
	  removeDiv=$('<div class="col-md-1 col-md-offset-9 removeTagDiv"><span class="glyphicon glyphicon-remove removeTag col-md-1 col-md-offset-11"></span></div>');
	  removeDiv.attr('id',_block.getId()+"remove");
      childBr=$('</br>');
  ...
  sy.createFun=function(){
	...
	$(document).on("blur","#div1 .block",sy.changeTitle);
	$(document).on("click",".removeTag",sy.removeBlock);
  }
  sy.hoverShow=function(){
	if($(this)[0].lastChild.id=="undefined"){
	  ...
	  $("#"+idStr).show();
	 // $("#"+idStr+"remove").show();
	  idstr=$(this).parent()[0].lastChild.id;
	  $("#"+idstr).show();
	}
  }
